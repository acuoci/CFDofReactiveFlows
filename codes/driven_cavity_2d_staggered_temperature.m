...
%        with inclusion of temperature equation (as a passive scalar)     %
...
alpha=0.1;          % thermal diffusivity [m2/s]
...
% Boundary conditions
Tn=300;       % north wall temperature [K]
Ts=400;       % south wall temperature [K]
Te=300;       % east wall temperature [K]
Tw=300;       % west wall temperature [K]
...
for is=1:nsteps
    ...
    T(2:nx+1,1)=2*Ts-T(2:nx+1,2);
    T(2:nx+1,ny+2)=2*Tn-T(2:nx+1,ny+1);
    T(1,2:ny+1)=2*Tw-T(2,2:ny+1);
    T(nx+2,2:ny+1)=2*Te-T(nx+1,2:ny+1);
    ...
    for i=2:nx+1
            for j=2:ny+1
                ...
                T(i,j)= To(i,j) + dt *( ...
                            (-uij/2/h+alpha/h^2)*To(i+1,j) + ...
                            ( uij/2/h+alpha/h^2)*To(i-1,j) + ...
                            (-vij/2/h+alpha/h^2)*To(i,j+1) + ...
                            ( vij/2/h+alpha/h^2)*To(i,j-1) + ...
                            (-4*alpha/h^2)*To(i,j) );
            end
    end
    ...
    surf(x,y,TT);
    axis('square');
    xlabel('x [m]');ylabel('y [m]'); title('temperature contour');
    hold off;
