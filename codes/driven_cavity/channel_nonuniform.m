...
for is=1:nsteps
    ...
    [ut, vt] = AdvectionDiffusion2D( ut, vt, u, v, nx, ny, hx, hy, dt, nu);
    ...
    [p, iter] = Poisson2D( p, ut, vt, gamma, nx, ny, hx, hy, dt, ...
                           beta, max_iterations, max_error, solver_type);
    
    % Correction on the velocity
    u(2:nx,2:ny+1)=ut(2:nx,2:ny+1)-(dt/hx)*(p(3:nx+1,2:ny+1)-p(2:nx,2:ny+1));
    v(2:nx+1,2:ny)=vt(2:nx+1,2:ny)-(dt/hy)*(p(2:nx+1,3:ny+1)-p(2:nx+1,2:ny));
...
% --------------------------------------------------------------------------------------
function [ut, vt] = AdvectionDiffusion2D( ut, vt, u, v, nx, ny, hx, hy, dt, nu)
