...
gamma(nx+1,nout_start:nout_end) = 1/3;
    ...
    if (strcmp(solver_type,'SOR'))
        ...
        for i=2:nx+1
            for j=2:ny+1
                ...
                I(counter) = k; J(counter) = k+1; V(counter) = -gamma(i,j); counter = counter+1;
                I(counter) = k; J(counter) = k-1; V(counter) = -gamma(i,j); counter = counter+1;
                I(counter) = k; J(counter) = k+(nx+2); V(counter) = -gamma(i,j); counter = counter+1;
                I(counter) = k; J(counter) = k-(nx+2); V(counter) = -gamma(i,j); counter = counter+1;

                b(k) = -gamma(i,j)*(h/dt)*(ut(i,j)-ut(i-1,j)+vt(i,j)-vt(i,j-1));
