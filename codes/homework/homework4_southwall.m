...
for istep=1:nsteps     
    ...
    % ------------------------------------------------------------------- % 
    [u,v] = ReconstructDimensionlessVelocity(u,v,psi,nx,ny,hx,hy,uwnorth,uwsouth);
    
    % ------------------------------------------------------------------- %
    % Find vorticity on boundaries
    % ------------------------------------------------------------------- %
    omega(:,1)  = (psi(:,1)-psi(:,2))*2/hy^2 +2/hy*uwsouth;         % south
    omega(1,:)  = (psi(1,:)-psi(2,:))*2/hx^2 ;                      % west
    omega(nx,:) = (psi(nx,:)-psi(nx-1,:))*2/hx^2 ;                  % east
    omega(:,ny) = (psi(:,ny)-psi(:,ny-1))*2/hy^2 -2/hy*uwnorth;     % north
    
    % ------------------------------------------------------------------- %
    % Advection-diffusion equation (new vorticity in interior points)
    % ------------------------------------------------------------------- %
    [omega] = AdvectionDiffusion2D(omega, u,v, Re, nx,ny, hx,hy, dtau);
...
%  ------------------------------------------------------------------------
function [f,iter] = Poisson2D(f,nx,ny,hx,hy,S,beta,max_iterations,max_error)
...
%  ------------------------------------------------------------------------
function [f] = AdvectionDiffusion2D(f, u,v, Re, nx,ny, hx,hy, dtau)
